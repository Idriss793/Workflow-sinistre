<div class="tab-pane fade" id="parties" role="tabpanel">
    <div class="container-fluid">
        ...
        <!-- Tableau des assurés principaux et parties adverses -->
        <div class="card card-clean mb-4">
            <div class="card-header bg-light text-center fw-bold fs-5">
                <i class="fas fa-user-shield me-2 text-primary"></i>Informations des parties
            </div>
            <div class="card-body">
                    ...
                    <table class="table table-hover table-bordered align-middle">
                        <thead class="table-light text-center">
                            <tr>
                                ...
                                <th>Matricule</th>
                                <th>Compagnie</th>
                                <th>Contact compagnie</th>
                            </tr>
                        </thead>
                        <tbody class="text-center">
                            {{-- Assurés principaux --}}
                            @forelse ($sinistres->assurePrincipals ?? [] as $index => $assure)
                                <tr>
                                    <td>{{ $index + 1 }}</td>
                                    <td>Assuré principal</td>
                                    <td>{{ $assure->nom ?? 'N/A' }}</td>
                                    <td>{{ $assure->prenom ?? 'N/A' }}</td>
                                    <td>{{ $assure->num_tel ?? 'N/A' }}</td>
                                    <td>{{ $assure->num_pol ?? 'N/A' }}</td>
                                    <td>{{ $assure->num_matri ?? 'N/A' }}</td>
                                    <td>-</td>
                                    <td>-</td>
                                </tr>
                            @empty
                                <tr>
                                    <td colspan="9" class="text-muted">Aucun assuré principal trouvé</td>
                                </tr>
                            @endforelse
                            ...
                            @forelse ($sinistres->assureTiers ?? [] as $index => $assure_tiers)
                                <tr>
                                    <td>{{ ($sinistres->assurePrincipals->count() ?? 0) + $index + 1 }}</td>
                                    <td>Partie adverse</td>
                                    <td>{{ $assure_tiers->nom_tiers ?? 'N/A' }}</td>
                                    <td>{{ $assure_tiers->prenom_tiers ?? 'N/A' }}</td>
                                    <td>{{ $assure_tiers->num_tel_tiers ?? 'N/A' }}</td>
                                    <td>{{ $assure_tiers->num_pol_tiers ?? 'N/A' }}</td>
                                    <td>{{ $assure_tiers->num_matri_tiers ?? 'N/A' }}</td>
                                    <td>{{ $assure_tiers->nom_assurance_tiers ?? 'N/A' }}</td>
                                    <td>{{ $assure_tiers->contact_assurance_tiers ?? 'N/A' }}</td>
                                </tr>
                            @empty
                                <tr>
                                    <td colspan="9" class="text-muted">Aucune partie adverse trouvée</td>
                                </tr>
                            @endforelse
                        </tbody>
                    </table>
